...
pub fn new_location_block(block_number: u64, data: LocationBlockData) -> CanonicalBlock {
    new_canonical_block(
        ...
        block_number,
        0,
        CanonicalData::Unknown(crate::serde::to_cbor_vec(&data).unwrap_or_default()),
    )
}
...
pub fn get_location_data(cblock: &CanonicalBlock) -> Result<LocationBlockData, LocationError> {
    if cblock.block_type == LOCATION_BLOCK {
        if let CanonicalData::Unknown(data) = cblock.data() {
            crate::serde::from_cbor_slice(data).map_err(|_err| LocationError::InvalidLocationBlock)
        } else {
...
mod tests {
    ...
    fn test_locblock_data_position_roundtrip() {
        ...
        let buf = crate::serde::to_cbor_vec(&data).unwrap();
        let data2 = crate::serde::from_cbor_slice(&buf).unwrap();
    ...
    fn test_cblock_location_roundtrip() {
        ...
        let cblock2 = crate::serde::from_cbor_slice(&buf).unwrap();
